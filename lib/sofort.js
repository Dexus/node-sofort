...
function Sofort(options) {
	...
	this.interface_version = 'node-sofort/0.1.2';
}
			...
			function (body, callback) {
				try {
					parseXmlString(body, function (err, result) {
						if (err || ! result) {
							return callback(new SofortException('Error postData', ['unknown API error']));
						}
						if (result.errors && result.errors.error && result.errors.error.length) {
							return callback(new SofortCodeException(result.errors.error[0].code));
						}
						if (result.new_transaction && result.new_transaction.transaction && result.new_transaction.payment_url) {
					...
					return callback(new SofortXmlException(e));
				}
			},
		], cb);
};
...
Sofort.prototype.postData = function (PaymentXml, cb) {
	...
	request({
		...
		if (response.statusCode === 401) {
			return cb(new SofortException('Error postData', ['401 Unauthorized']));
		}
		if (response.statusCode === 404) {
			return cb(new SofortException('Error postData', ['404 Not Found']));
		}
		cb(new SofortException('Error postData', ['unknown error']));
	});
};
...
Sofort.prototype.parseNotification = function (xmlBody, cb) {
	...
	async.waterfall([
			...
			function (result, callback) {
				...
				self.postData(BuildXML.render(NotifiObj), callback);
			},
			function (body, callback) {
				try {
					parseXmlString(body, function (err, result) {
						...
						if (result.transactions && result.transactions.transaction_details) {
							return callback(null, result.transactions.transaction_details[0]);
						}
						return callback(new SofortException('Error postData', ['unknown API error']));
					});
				} catch (e) {
...
function SofortXmlException(error) {
	this.originalError = error;
	this.message = error.name + ' -> ' + error.message;
	this.name = "SofortXmlException";
}
SofortXmlException.prototype = Error.prototype;
